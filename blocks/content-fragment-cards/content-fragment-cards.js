 ...
 * @param {string} contentPath - CF path
 * @param {string} variationname - variation name
 * @param {object} env - { aemauthorurl, aempublishurl, isAuthor }
 * @returns {Promise<object|null>} CF item or null
 */
...
/**
 * Build one card DOM (image + body with title, description, CTA) from CF item.
 * @param {object} cfReq - CF item from GraphQL
 * @param {string} contentPath - CF path (for data-aue-resource)
 * @param {string} variationname - variation name
 */
async function buildCardElement(cfReq, contentPath, variationname, env, ctaStyle = 'button') {
  ...
  imageDiv.className = 'content-fragment-cards-card-image';
  if (imgUrl) {
  ...
  bodyDiv.className = 'content-fragment-cards-card-body';
  const title = document.createElement('h3');
  title.textContent = cfReq?.title ?? '';
  title.setAttribute('data-aue-prop', 'title');
  const subtitle = document.createElement('p');
  subtitle.classList.add('content-fragment-cards-card-subtitle');
  subtitle.textContent = cfReq?.subtitle ?? '';
  subtitle.setAttribute('data-aue-prop', 'subtitle');
  const desc = document.createElement('div');
  desc.classList.add('content-fragment-cards-card-description');
...
/**
 * Decorate content-fragment-cards block: each row = one content fragment card (CF path + optional variation).
 * Renders a cards container (ul > li) with each li filled from the CF API (same as content-fragment block).
 */
export default async function decorate(block) {
  ...
  for (const row of rows) {
    ...
    const li = await buildCardElement(cfReq, contentPath, variationname, env, ctaStyle);
